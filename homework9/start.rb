...
class Start
  ...
  def stations_management
    loop do
      item_stations = @interface.station_management_menu
      break if item_stations.zero?

      stations_management_menu(item_stations)
    end
  end

  def stations_management_menu(item_stations)
    case item_stations
    when 1
      create_station
    when 2
    ...
    if @routes[number_route].nil?
      ...
      if name_add_station
        @interface.station_removed if
            @routes[number_route].del_station(name_add_station)
      else
        @interface.not_station_found
      end
    end
  ...
  def create_train
    @interface.train_type_choice_message
  ...
  def create_wagons
    ...
    if number.zero?
      ...
      case type
      when 1 then
        create_wagon_cargo(number)
      when 2 then
        create_wagon_passenger(number)
      end
    end
  end
